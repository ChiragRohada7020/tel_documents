...
This repository includes `render.yaml` and a Docker image with Tesseract OCR. Create a Render Blueprint from the repository; it deploys as a free web service and receives Telegram updates through a webhook. Set `TELEGRAM_BOT_TOKEN`, `MONGODB_URI`, and `GROQ_API_KEY` in Render's environment-variable screen. After the first deploy, set `WEBHOOK_URL` to `https://<your-service>.onrender.com/<your Telegram bot token>` and redeploy. Do not commit `.env` or any credentials.
